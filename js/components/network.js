...
import { mockUsers, mockConnections, mockCurrentProfile } from '../mockData.js';
...
export async function renderNetworkBoard(departmentFilter = 'All', searchQuery = '') {
  ...
  try {
    ...
    if (response.documents.length === 0) {
      renderMockNetworkBoard(departmentFilter, searchQuery);
      return;
    }
    ...
    response.documents.forEach(creator => {
      ...
      card.innerHTML = `
        ...
        <div class="talent-tags" style="display: flex; gap: 6px; flex-wrap: wrap; margin-bottom: 16px;">
            ...
            <span class="talent-tag" style="background: var(--surface-2); padding: 4px 8px; border-radius: 4px; font-size: 0.75rem;">${creator.mutualConnections || 0} mutuals</span>
        </div>
    ...
    console.warn('Network fallback to mock profiles.');
    renderMockNetworkBoard(departmentFilter, searchQuery);
  }
}
...
export async function openTalentProfile(creatorId) {
  ...
  try {
    ...
    console.warn('Talent profile fallback unavailable.');
...
function renderMockNetworkBoard(departmentFilter = 'All', searchQuery = '') {
  if (!talentGrid) return;

  const myProfile = StorageService.get('kalakar_user_profile') || mockCurrentProfile;
  const query = String(searchQuery || '').trim().toLowerCase();

  const users = mockUsers.filter((creator) => {
    if (myProfile && creator.$id === myProfile.$id) return false;
    if (departmentFilter !== 'All' && creator.primaryCraft !== departmentFilter) return false;
    if (!query) return true;
    return creator.name.toLowerCase().includes(query) || creator.primaryCraft.toLowerCase().includes(query) || creator.city.toLowerCase().includes(query);
  });
  ...
  users.forEach((creator) => {
    const connection = mockConnections.find((item) => item.userId === creator.$id);
    const mutual = connection?.mutual || creator.mutualConnections || 0;
    ...
    card.innerHTML = `
      <div style="display: flex; gap: 16px; align-items: center; margin-bottom: 12px;">
        <img src="https://i.pravatar.cc/100?u=${creator.$id}" style="width: 60px; height: 60px; border-radius: 50%; object-fit: cover; border: 2px solid var(--line);">
        <div>
          <h3 style="margin:0; font-size: 1.1rem;">${creator.name} ${creator.isVerified ? '<span style="color:var(--brand-gold);">✓</span>' : ''}</h3>
          <p class="meta" style="font-size: 0.85rem;">${creator.primaryCraft} · ${creator.city}</p>
        </div>
      ...
      <div class="talent-tags" style="display: flex; gap: 6px; flex-wrap: wrap; margin-bottom: 16px;">
        <span class="talent-tag" style="background: var(--surface-2); padding: 4px 8px; border-radius: 4px; font-size: 0.75rem;">⭐ ${creator.vouchCount || 0} Vouches</span>
        <span class="talent-tag" style="background: var(--surface-2); padding: 4px 8px; border-radius: 4px; font-size: 0.75rem;">${mutual} mutual connections</span>
      </div>
      <button class="primary action-gold mock-connect-btn" style="width: 100%; border-radius: 20px; font-weight: 600;">Connect</button>
    `;

    card.querySelector('.mock-connect-btn')?.addEventListener('click', (event) => {
      event.stopPropagation();
      showToast(`Connection request sent to ${creator.name}`, 'success');
    });
    card.addEventListener('click', () => openTalentProfile(creator.$id));
    talentGrid.appendChild(card);
  });
}
